...
Usage:
    python3 benchmarks/gen_parse_bench.py --num-funs 60000 -o benchmarks/parse_bench.psh

By default every generated function is called once at the end. Pass --num-exec
to only call the first N of them, which isolates parsing cost from run time.
"""
...
def generate(num_funs, seed, max_depth, max_stmts, num_exec=None):
    rng = random.Random(seed)
    gen = Generator(rng, max_depth)

    if num_exec is None:
        num_exec = num_funs
    num_exec = min(num_exec, num_funs)

    gen.emit("// Auto-generated by benchmarks/gen_parse_bench.py -- do not edit by hand.")
    gen.emit("// " + str(num_funs) + " randomly-generated functions, " +
             str(num_exec) + " of which are called at the end.")
    ...
    gen.emit("// Call the first " + str(num_exec) + " functions once")
    for name, num_params in funs[:num_exec]:
...
def main():
    ...
    parser.add_argument("-n", "--num-funs", type=int, default=60000)
    parser.add_argument("--num-exec", type=int, default=None,
                        help="Only call the first N functions (default: all of them)")
    ...
    source = generate(args.num_funs, args.seed, args.max_depth, args.max_stmts, args.num_exec)

    with open(args.output, "w") as file:
        file.write(source)

    num_exec = min(args.num_exec, args.num_funs) if args.num_exec is not None else args.num_funs
    num_lines = source.count("\n")
    print("Wrote {} ({} functions, {} executed, {} lines, {:.1f} MiB)".format(
        args.output, args.num_funs, num_exec, num_lines, len(source) / (1024 * 1024)))
